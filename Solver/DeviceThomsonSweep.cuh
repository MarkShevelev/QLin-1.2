...
namespace iki {
	template<typename T>
	__global__ void thomson_sweep_forward_kernel(
	...
	template<typename T>
	__global__ void thomson_sweep_backward_kernel(
		iki::DeviceArray<T, 2u> a,
		iki::DeviceArray<T, 2u> b,
		iki::DeviceArray<T, 2u> c,
		iki::DeviceArray<T, 2u> d,
		...
		for (size_t y_idx = y_size - 3; y_idx != 0; --y_idx) {
			T w = c(y_idx, x_idx) / b(y_idx + 1, x_idx);
			b(y_idx, x_idx) = fma(-w, a(y_idx + 1, x_idx), b(y_idx, x_idx));
			d(y_idx, x_idx) = fma(-w, d(y_idx + 1, x_idx), d(y_idx, x_idx));
		}
		res(1, x_idx) = d(1, x_idx) / b(1, x_idx);

		for (size_t y_idx = 2; y_idx != y_size - 1; ++y_idx) {
			res(y_idx, x_idx) = fma(-a(y_idx, x_idx), res(y_idx - 1, x_idx), d(y_idx, x_idx)) / b(y_idx, x_idx);
		}
	}
}/*iki*/